...
tar --exclude='./node_modules' --exclude='./.git'  --exclude='./output' --exclude='./dist' --exclude='./ci.yml' --exclude='./swc' --exclude='./idea' --exclude="./vscode" --exclude='./.rules/__tests__' --exclude='./.rules/package.json' --exclude='./.rules/tsconfig.json' -cf - . | tar -C "$OUTPUT_DIR" -xf -
...
git config user.email "luna@example.com"
...
git commit -m "Initial Luna project setup with Taro weapp template $AGILE_REVISION"
