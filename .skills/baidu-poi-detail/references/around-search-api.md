...
| Endpoint | `GET https://app-b9plzy10uj29-api-DLEO7eMnzMwa-gateway.appmiaoda.com/place/v3/around` |
| Auth | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>`（platform_managed） |
| Content-Type | 无请求体（GET 参数通过 URL 查询串传递） |
| Third-party domain | `app-b9plzy10uj29-api-DLEO7eMnzMwa-gateway.appmiaoda.com` |
...
async function searchAround(
  ...
  const response = await fetch(
    `https://app-b9plzy10uj29-api-DLEO7eMnzMwa-gateway.appmiaoda.com/place/v3/around?${params.toString()}`,
    {
      method: "GET",
      headers: {
